...
通过 DeepSeek API（OpenAI 兼容）对语音识别文本进行全文精修：
1. 自动检测语言（中文/英文），使用对应的润色策略
2. 修正 ASR 同音错别字（在/再、的/得/地 等）
3. 同义词优化，选择最贴切的表达
4. 逻辑重构：梳理因果/转折/并列关系，拆分长句，合并重复
5. 去除口语填充词（内置 + 用户自定义）
6. 识别自我修正，保留最终意图
7. 自动格式化列表/步骤为结构化文本
8. 长语音自动提炼要点并编号
9. 对话记忆（few-shot）保持风格一致
"""
...
5. **Logical restructuring**:
   ...
   - Important: Preserve original meaning, do NOT add information not present in the input

6. **Auto-formatting**:
   - Detect enumerated points → convert to numbered lists
