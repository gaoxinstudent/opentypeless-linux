...
- ✨ **AI 全文精修**：DeepSeek 自动断句加标点、修正同音错别字、优化同义词、重构逻辑、去填充词、格式化列表
- 🌐 **中英双语**：自动检测语音语言，中文修正「在/再」同音错字，英文修正「their/there」混淆
- 🛠️ **自定义屏蔽词**：可追加自定义填充词（如「就是说」「好不好」），内置列表自动去重
- 📋 **长语音结构化**：长段语音自动提炼要点，输出「一、二、三…」编号格式
...
| 文本润色 | DeepSeek V4 Flash | 7 步全文精修：断句标点→错字修正→同义词→去填充→改口→逻辑→格式化 |
| 语言检测 | 启发式规则 | CJK 字符存在 → 中文；纯拉丁 → 英文 |
| 对话记忆 | JSON 持久化 | few-shot 示例注入 system prompt |
| 自定义屏蔽 | 配置 + prompt 注入 | 中文顿号分隔，英文逗号分隔，自动去重 |
...
### v0.2.0 (2026-06-21) — 润色增强

**全文精修升级**
- 润色从「去填充词+格式化」升级为 **7 步全文精修**
- 新增**断句与标点**（第 1 优先级）：自动添加句号/逗号/顿号/分号/引号/冒号
- 新增**错别字修正**：修正 ASR 同音错字（在/再、的/得/地、反映/反应 等）
- 新增**同义词优化**：替换不贴切词汇（弄→处理、搞→完成 等）
- 新增**逻辑重构**：梳理因果关系、拆分长句、合并重复、调整语序

**自定义屏蔽词**
- 新增 `custom_fillers_zh` / `custom_fillers_en` 配置项
- 设置界面支持自定义填充词追加（中文顿号分隔、英文逗号分隔）
- 内置列表与自定义列表自动去重

**长语音结构化**
- 新增 `auto_structure_long` 配置项
- 长语音自动提炼核心要点，输出「一、二、三…」编号格式

**模型切换**
- 默认模型改为 `deepseek-v4-flash`（更快更便宜）
- 对话记忆机制：历史润色记录作为 few-shot 示例
